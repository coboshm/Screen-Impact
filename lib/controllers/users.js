...
exports.create = function(req, res, next){
  ...
  newUser.save(function(err, user){
    ...
    if (err && err.code == 11000){
      ...
      return res.render('users/new', {done: false, user : newUser, error: true, error_msg: req.flash('error'), active : 'register'});
    }
    if(err) return next(err);
...
exports.userValidations = function(req, res, next){
  ...
  if (validationErrors.length > 0){
    req.flash('error', 'There are incorrect fields');

    // Create handling if errors present
    if (creatingUser) return res.render('users/new', {done: false, user : new User(req.body), error: true, error_msg: req.flash('error'), active : 'register'});
