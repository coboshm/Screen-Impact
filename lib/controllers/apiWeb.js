    ...
    if (list_assets.length > 0 && list_mac.length > 0) {
        if (req.body.type) {
            ...
            Impact.find({
                ...
                reslt = set_zero_dates_motion(req.body.from, req.body.to, results);
                return res.json(reslt);
            });
        }
...
function set_zero_dates_motion(from, to, results) {
    var res = results.slice();
    ...
    if (results.length > 0 ) {
        ...
        for (var j = 0; j < results.length - 1; ++j) {
            if (j >= 1) {
                ...
                if (results[j].createdAt < new Date(results[j - 1].createdAt.getTime() + 6000) && results[j].createdAt >= new Date(results[j + 1].createdAt.getTime() - 6000) &&
                    results[j].motion == results[j - 1].motion && results[j].motion == results[j + 1].motion) {
                    res.splice(j + aux, 1);
                    aux--;
                }
            }
